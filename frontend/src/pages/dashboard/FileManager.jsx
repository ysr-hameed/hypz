...
const FileManager = () => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            for (let i = 0; i < selectedFiles.length; i++) {
                    ...
                    try {
                        await uploadFilePresigned(
                            ...
                            (fileProgress) => {
                                const totalProgress =
                                    fileProgressStart +
                                    (fileProgress / 100) * fileProgressRange;
                        ...
                        logger.warn(
                            "Presigned upload failed, falling back to traditional upload:",
                            error
                        );
                        await uploadFileTraditional(
                            file,
                            bucketId,
                            fileProgressStart,
                            fileProgressRange
                ...
                setUploadProgress(
                    Math.round(fileProgressStart + fileProgressRange)
                );
            }
    ...
    const uploadFileTraditional = async (
        ...
        const progressInterval = setInterval(() => {
            ...
            if (simulatedProgress < 90) {
                const totalProgress =
                    progressStart + (simulatedProgress / 100) * progressRange;
                setUploadProgress(Math.round(totalProgress));
            }
        }, 100);
        ...
        try {
            await fileAPI.upload(bucketId, formData, (progressEvent) => {
                ...
                if (progressEvent && progressEvent.total) {
                    ...
                    const totalProgress =
                        progressStart + (fileProgress / 100) * progressRange;
                    setUploadProgress(Math.round(totalProgress));
                }
            });
        } finally {
    ...
    const handleDownload = async (fileId, fileName) => {
        try {
            ...
            // Use fetch API with proper authorization headers
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-950 p-6">
                                    ...
                                    <div
                                        className={`${
                                            viewMode === "grid"
                                                ? "w-16 h-16"
                                                : "w-12 h-12"
                                        } bg-gradient-to-br from-primary-100 to-purple-100 dark:from-primary-900/30 dark:to-purple-900/30 rounded-lg flex items-center justify-center group-hover:scale-110 transition`}>
                                        <FolderOpen
                    ...
                    {files.map((file) => (
                        <div
                            ...
                            <div className="flex items-start justify-between mb-3">
                                <div
                                    className={`w-12 h-12 rounded-lg flex items-center justify-center ${getFileColor(
                                        file.mime_type
                                    )}`}>
                            ...
                            <h3
                                className="font-medium text-gray-900 dark:text-white text-sm mb-1 truncate"
            ...
            {detailsModal.isOpen && detailsModal.item && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
                    <div className="bg-white dark:bg-gray-900 rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-200 dark:border-gray-700">
                        ...
                        <div className="space-y-4">
                            <div
                                className={`w-20 h-20 mx-auto rounded-lg flex items-center justify-center ${getFileColor(
                                    detailsModal.item.mime_type
                                )}`}>
                            ...
                            <div className="space-y-3">
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-gray-900 dark:text-white break-all">
                                        {detailsModal.item.original_name}
                                    </p>
                                </div>
                                <div>
                                    <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-mono text-gray-900 dark:text-white">
                                        {formatBytes(detailsModal.item.size)}
                                    </p>
                                </div>
                                <div>
                                    <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2">
                                        <p className="text-sm text-gray-900 dark:text-white font-mono truncate flex-1">
                                            {detailsModal.item.url}
                                        </p>
                                        <button
                                            onClick={() =>
                                                copyToClipboard(
            ...
            <ConfirmModal
                ...
                title={`Delete ${
                    confirmModal.type === "bucket" ? "Bucket" : "File"
                }`}
