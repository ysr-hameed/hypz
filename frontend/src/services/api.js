...
export const authAPI = {
    ...
    verifyEmail: (token) =>
        api.post(apiConfig.endpoints.verifyEmail, { token }),
    resendVerification: (email) =>
        api.post(apiConfig.endpoints.resendVerification, { email }),
    forgotPassword: (email) =>
        api.post(apiConfig.endpoints.forgotPassword, { email }),
    resetPassword: (token, password) =>
        api.post(apiConfig.endpoints.resetPassword, { token, password }),
    getCurrentUser: () => api.get(apiConfig.endpoints.getCurrentUser),
    logout: (refreshToken) =>
        api.post(apiConfig.endpoints.logout, { refreshToken }),
...
export const fileAPI = {
    ...
    getAll: (bucketId, params) =>
        api.get(apiConfig.endpoints.getFiles(bucketId), { params }),
    getById: (fileId) => api.get(apiConfig.endpoints.getFile(fileId)),
    download: (fileId) => {
        // Create download URL with proper authorization
        // The backend authMiddleware supports both Authorization header and query token
        const token = localStorage.getItem("token");
        if (token) {
            return Promise.resolve({
                downloadUrl: `${
                    apiConfig.API_URL
                }${apiConfig.endpoints.downloadFile(fileId)}`,
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
        }
        // Fallback to query param if no token in storage (shouldn't happen)
        return Promise.resolve({
            downloadUrl: `${
                apiConfig.API_URL
            }${apiConfig.endpoints.downloadFile(fileId)}?token=${token}`,
        });
    },
    delete: (fileId) => api.delete(apiConfig.endpoints.deleteFile(fileId)),
    update: (fileId, data) =>
        api.patch(apiConfig.endpoints.updateFile(fileId), data),
};
...
export const usageAPI = {
    ...
    getHistory: (days) =>
        api.get(apiConfig.endpoints.usageHistory, { params: { days } }),
...
export const twoFactorAPI = {
    ...
    verify2FALogin: (email, code, useBackupCode = false) =>
        api.post("/auth/2fa/verify-login", { email, code, useBackupCode }),
...
export const userAPI = {
    ...
    updateNotificationPreferences: (data) =>
        api.put("/user/notifications", data),
...
export const notificationAPI = {
    ...
    deleteNotificationAdmin: (notificationId) =>
        api.delete(`/notifications/admin/${notificationId}`),
};
...
export const adminAPI = {
    ...
    getWebhookDeliveries: (subscriptionId, params) =>
        api.get(`/admin/webhooks/${subscriptionId}/deliveries`, { params }),
    disableWebhook: (subscriptionId) =>
        api.patch(`/admin/webhooks/${subscriptionId}/disable`),
