...
export const createSubscription = asyncHandler(async (req, res) => {
    ...
    logger.info(
        { userId, planId, variantId, body: req.body },
    ...
    // If no variantId provided, try to get it from the plan's skydo_variant_id
    if (!variantId && plan.skydo_variant_id) {
        variantId = plan.skydo_variant_id;
        logger.info(
            { variantId, source: "database" },
            "Using variant ID from plan"
        );
    }

    // If still no variant ID, plan is not purchasable
    if (!variantId) {
        logger.error({ planId, plan }, "No skydo_variant_id found for plan");
    ...
    // Convert variant ID to string (Skydo expects string IDs)
    const variantIdString = String(variantId);

    // Create Skydo checkout with custom data
    try {
        logger.info(
            { variantId: variantIdString, planId, userId },
            "Creating Skydo checkout"
        );

        const checkoutData = await createSkydoCheckout(variantIdString, {
       ...
       VALUES ($1, $2, $3, $4)`,
        ...
        try {
          ...
          payment_gateway, transaction_id, metadata, invoice_url
            ...
            logger.error(
                { err, userId, planId, checkoutId: checkoutData.id },
                "Failed to create pending payment record"
            );
        }
        ...
        logger.error(
            { err: error, planId, variantId },
            "Failed to create checkout"
        );
        return errorResponse(
            res,
            `Failed to create checkout: ${error.message}. Please verify Skydo configuration.`,
            400
        );
    }
});
...
export const addPaymentMethod = asyncHandler(async (req, res) => {
    ...
    // Store payment method - NOTE: Table uses skydo_payment_method_id
    const result = await query(
        `INSERT INTO payment_methods (user_id, skydo_payment_method_id, type, brand, last4, exp_month, exp_year, is_default)
...
export const getCurrentUsageAndCost = asyncHandler(async (req, res) => {
    ...
    if (plan.type === "payg") {
        ...
        const metaOps =
            Number(usage.total_upload_calls || 0) +
        ...
        accessOpsCost =
            (accessOps / 1000) * (plan.payg_access_ops_rate || 0.004);
    }
...
export const payPendingInvoice = asyncHandler(async (req, res) => {
    ...
    // Here you would integrate with Skydo to charge the card
    ...
    await transaction(async (client) => {
      ...
      RETURNING *`,
            [
                ...
                {
                    billingPeriod: {
                        start: billing.billing_period_start,
                        end: billing.billing_period_end,
                    },
                },
            ]
        );
       ...
       WHERE id = $2`,
        ...
        await client.query(
            ...
            [
                userId,
                "manual_payment",
                { billingId, amount: billing.total_cost },
            ]
        );
    });
    ...
    successResponse(
        res,
        { paid: true },
        "Payment successful. Services reactivated."
    );
});
